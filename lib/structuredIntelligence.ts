...
export function buildInfrastructureFinanceWatch(stories: IntelligenceStory[]): IntelligenceItem[] {
  ...
  const capital = /\b(financ(?:e|ing)|funding|investment|invests?|capital expenditure|capex|debt|loan|bond|private equity|infrastructure fund|partnership|agreement|commitment|spending|buildout|construction project)\b/i;
  ...
  return uniqueItems(
    stories.filter((story) => {
      ...
      const financingType = firstMatch(input, [
        [/\bdebt|loan|bond\b/i, "Debt financing"],
...
export function buildPublisherLicensingLedger(stories: IntelligenceStory[]): IntelligenceItem[] {
  ...
  const relationship = /\b(licens(?:e|ed|ing)|content agreement|training.data|attribution|revenue.sharing|usage.payment|partnership|settlement|lawsuit|copyright|negotiat(?:e|ing|ions)|deal)\b/i;
